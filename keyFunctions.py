...
def generateMetaDataFile(call, BW=None):
    #A meta data file is generated upon the sucessful conclusion of a scan by the rtl_power_fftw software.
    #However, if we do a short scan and end it manually, no meta data file is generated.
    #To be able to plot the data later, we need a meta data file with at least the frequencies and
    #number of bins.
    #The input is a call generated by the makeScanCall function.
    #Output is a file with this format:

    # Generated before scan
    # 140 # frequency bins (columns)
    # 962 # scans (rows) <--we don't know this, enter 0
    # 1226600000 # startFreq (Hz)
    # 1228585714 # endFreq (Hz)
    # 14285 # stepFreq (Hz) <--this is from the hardware, use the detected bandwidth
    # 0.0007 # effective integration time secs <--we don't know this, enter 0
    # 0.00935551 # avgScanDur (sec) <--we don't know this, enter 0
    # 2021-04-07 06:17:59 UTC # firstAcqTimestamp UTC <--we don't know this, enter 0
    # 2021-04-07 06:18:59 UTC # lastAcqTimestamp UTC <--we don't know this, enter 0
    import os
    import datetime
    ...
    metaFileName = fileName+'.met'
    #We should have what we need. Write out the file.
    with open(metaFileName, 'w') as metaFile:
        metaFile.write('Generated before scan\n')
        metaFile.write(str(numBins)+' # frequency bins (columns)\n')
        metaFile.write('0 # scans (rows)\n')
        metaFile.write(hzLow+' # startFreq (Hz)\n')
        metaFile.write(hzHigh+' # endFreq (Hz)\n')
        metaFile.write(str(BW)+' # stepFreq (Hz)\n')
        metaFile.write('0 # effective integration time secs\n')
        metaFile.write('0 # avgScanDur (sec)\n')
        metaFile.write(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S %Z')+' UTC # firstAcqTimestamp UTC \n')
        metaFile.write(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S %Z')+' UTC # lastAcqTimestamp UTC \n')
    print('Generated meta data file '+metaFileName)
